...
ggplot(aes(x = num_uni_topic, y = estimate, 
             color = as.factor(GPAO_included), shape = as.factor(significant))) + 
  geom_point(size = 5) + 
...
#__plot (raw grades) ####
  ...
  scale_fill_manual(values = c("#66c2a5", "white", "white","white","white")) + 
...
#reorder course topics
sai$crs_topic <- factor(sai$crs_topic, 
                               levels = c("Genetics", "CellBio"))
#relabel order of institutions
sai$uni_label <- recode_factor(sai$university,
                                      "ASU" = "A", "UCD" = "B",
                                      "IU" = "C", "Purdue" = "D", 
                                      "UM" = "E")
levels(sai$uni_label) <- c("A", "B", "C","D","E")

sai %>%
  filter(category == "all") %>%
  ggplot(aes(x = SAI, y = mean_grade_anomaly, fill = uni_label)) + 
  geom_point(aes(size = n),
             position = position_dodge(0.2), 
             alpha = 0.8, color = "black", pch = 21) +
  geom_hline(yintercept = 0) + 
  ...
  scale_size_continuous(trans = "log10", breaks = c(10,100,1000))+ #point size for readability
  scale_fill_manual(values = alpha_colors) + 
  labs(y = "Mean Grade Anomaly", color = "Institution", fill = "Institution",
       x = "No. of Systemic Advantages", size = "N") +
  ylim(-1.10, 0) + 
  theme_minimal(base_size = 20) + 
  theme(axis.text = element_text(color = "black", size = 20), 
        strip.text = element_text(color = "black", size = 24), 
