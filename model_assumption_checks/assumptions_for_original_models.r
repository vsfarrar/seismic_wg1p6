...
library(xlsx)
model_1 = numgrade ~ gpao + female + as.factor(ethnicode_cat) + firstgen + transfer + lowincomeflag + international + (1|crs_term)
model_2 = numgrade ~ gpao + female + urm + firstgen + transfer + lowincomeflag + international + (1|crs_term)
...
assumptions = function(clean_data = dat_new, model){
  ...
  shapiro_1 = sapply(st.res, shapiro.test)
  ...
  ## Testing for level 1 residuals if p>.05 then the assumption is tenable
  shapiro_2 = sapply(st.ran, shapiro.test)
  ...
  if (model == model_1){
    mod_vars = c("crs_term","gpao", "female", "ethnicode_cat", "firstgen", "transfer", "lowincomeflag", "international")
  } else if (model == model_2){
    mod_vars = c("crs_term","gpao", "female", "urm", "firstgen", "transfer", "lowincomeflag", "international")
  } 
  ## Level 1 residuals
  #===================
