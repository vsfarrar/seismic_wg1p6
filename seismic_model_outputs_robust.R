...
main_fx_all_rob <-
  ...
  mutate(fit = map(data, ~rlmer(numgrade ~ gpao + female + as.factor(ethniccode_cat) + firstgen + transfer + lowincomeflag +
                               + (1|crs_offering), data = ., method = "DAStau")), 
...
main_fx_no_gpao_rob <-
  ...
  mutate(fit = map(data, ~rlmer(numgrade ~ female + as.factor(ethniccode_cat) + firstgen + transfer + lowincomeflag +
                                + (1|crs_offering), data = ., method = "DAStau")), 
         results = map(fit, tidy)) %>%
  unnest(results) %>%
  select(-data, -fit, - group) %>%
...
#  dat_new %>%
#  group_by(university,crs_name) %>% nest() %>%
#  mutate(fit = map(data, ~rlmer(numgrade ~ gpao + female + urm + firstgen + transfer + lowincomeflag +
#                                + (1|crs_offering), data = ., method = "DAStau")), 
#         results = map(fit, tidy)) %>%
#  unnest(results) %>%
#  select(-data, -fit, - group) %>%
